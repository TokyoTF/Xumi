...
(function () {
  ...
  apply(state);
  updateBg();
  new MutationObserver(() => { injectProfileItem(state); apply(state); updateBg(); }).observe(document.body, { childList: true, subtree: true });
  setInterval(() => { injectProfileItem(state); apply(state); updateBg(); closeNowPlayingOnStart(state); injectWindowControls(state); }, 1000);
